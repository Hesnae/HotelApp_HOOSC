...
public class NfcTagViewModel extends ViewModel {
    ...
    private NdefMessage createNdefForUrl(String url) {
        NdefRecord record = NdefRecord.createUri(url);
        return new NdefMessage(new NdefRecord[] { record });
    }
    private NdefMessage createNdefForPhone(String PhoneNumber){
        NdefRecord record = NdefRecord.createUri("tel:" + PhoneNumber);
        return new NdefMessage(new NdefRecord[] { record });
    }
    ...
    private void write(NdefMessage ndefMessage, Ndef ndef) {
        if (ndef != null) {
            try {
                ...
                Log.e("MEL", "WRITTEN " + ndefMessage.getRecords()[0].getPayload().toString());
